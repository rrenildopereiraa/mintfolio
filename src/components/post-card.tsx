...
export function PostCard({ post }: { post: Post }) {
	return (
		<Link
			href={`/blog/${post.slug}`}
			className="d-f fd-c g-2 mx--3 px-3 py-4 br-lg td-none c-zinc-9 h:bg-silver-1 tp-a tdu-150 fv:os-s fv:oo-2 fv:oc-mint @sm:fd-r @sm:ai-b @sm:g-6"
		>
			{/* Fixed width on wide screens so short dates don't shunt the titles
			    left and leave the column ragged. */}
			<time
				dateTime={post.date}
				className="fs-0 fs-xs c-slate @sm:w-24"
				style={MONO_STYLE}
			>
			...
			<span className="d-f fd-c g-1">
				<span className="fw-600 fs-md lh-3 c-zinc-9">{post.title}</span>
				<span className="fs-sm lh-5 c-slate">{post.description}</span>
			</span>
		</Link>
	);
}
