	...
	return (
		<PreviewCard.Root>
			<PreviewCard.Trigger
				...
				// icon dropped inline.
				className="d-if ai-c g-2 va-m c-zinc-9 c-p"
				render={<span />}
			>
				{children}
			</PreviewCard.Trigger>
			...
			<PreviewCard.Portal>
				...
				<PreviewCard.Positioner sideOffset={10} style={{ zIndex: 50 }}>
					<PreviewCard.Popup
						className="p-r p-4 br-lg bw-1 bs-s bc-silver-2 bg-white"
						// Yumma has no box-shadow utility, so the lift is inline.
						style={{
							width,
							boxShadow: "0 16px 38px -18px rgba(48, 48, 53, 0.32)",
						}}
					>
...
function Arrow() {
	return (
		<svg width="14" height="7" viewBox="0 0 14 7" aria-hidden="true">
			<title>Tip</title>
			<path d="M0 0 7 7 14 0" fill="#ffffff" />
			<path d="M0 0 7 7 14 0" fill="none" stroke="#e1e3e7" strokeWidth="1" />
		</svg>
	);
}
