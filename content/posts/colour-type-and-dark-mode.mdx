export const meta = {
	title: "Colour, type, and how dark mode works here",
	description:
...
## The colours no class can reach

A few things are not properties Yumma has a utility for: gradient stops, box-shadow tints, an SVG `fill`. Those live in `src/lib/colors.ts`, still as `light-dark()` pairs:
...
No custom stylesheet. `app/globals.css` is the font imports and one `@yummacss` directive, and `pnpm validate` checks every class in the project against Yumma so a typo is a build failure rather than a silent no-op.
