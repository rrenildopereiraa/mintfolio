...
Fenced blocks are highlighted at build time by [Shiki](https://shiki.style), which means the colours are baked into the HTML. No highlighter ships to the browser, and nothing flashes grey before it paints:
...
Name the language after the backticks. A block with no language still renders, just without colour. Long lines scroll inside the block rather than stretching the page.

Both themes are highlighted from one pass — the same markup carries light and dark colours, so switching themes does not re-render anything.

## Tables and lists

GitHub-flavoured markdown is on, so tables work:
